...
class Cross_validate_GridSearch:
    ...
    def EvaluateDifferentModels(self,trainset,trainlabel,testset,testlabel):
        ...
        pipeline  = sample_pipeline([("HandleMissingValues",HandleMissingValues()),("sampling",SMOTE(random_state=50)),("Normalize",Normalize()),("VarianceFilter",VarianceFilter()),("CorrelationFilter",Correlation_Filter()),('feature_sele',FeatureSelector()),('Ensemble',ensemble)])
        pipeline.fit(trainset,trainlabel)
        y_pred_train=pipeline.predict(trainset)
        y_pred_ensemble=pipeline.predict(testset) 
        y_pred_prob_ensemble=pipeline.predict_proba(testset)
        print("ACCURACY ON TRAIN SET :",accuracy_score(trainlabel,y_pred_train))
    ...
    def EvaluateDifferentModelswithPCA(self,trainset,trainlabel,testset,testlabel):
        ...
        ensemble = VotingClassifier(best_models,n_jobs=-1,voting='soft')
        Saving_Model.append(ensemble) # Appending Voting model
        pipeline  = sample_pipeline([("HandleMissingValues",HandleMissingValues()),("Normalize",Normalize()),("VarianceFilter",VarianceFilter()),("CorrelationFilter",Correlation_Filter()),('feature_sele',FeatureSelector()),("PCA",pca()),('Ensemble',ensemble)])
        pipeline.fit(trainset,trainlabel)
        y_pred_train=pipeline.predict(trainset)
        y_pred_ensemble=pipeline.predict(testset) 
        y_pred_prob_ensemble=pipeline.predict_proba(testset)
        ...
        F1_scores_.append(F1_score_GNBLR[0])
        F1_scores_.append(F1_score_GNBLR[1])
        fig = plt.figure()
        print("Models used",len(Models_used))
        print("Length of F1_scores_",len(F1_scores_))
        try:
                ax = fig.add_axes([0,0,1,1])
...
class Novel_Predictions:
    ...
    def Mergealldata(self,data,data_labels,Novel_data):
        print(data.shape)
        ligand=Novel_data["Ligand"]
        Novel_data=Novel_data.drop(["Ligand"],axis=1)
        Novel_data_=Novel_data[data.columns]
        Novel_data['Ligand']=ligand
        return X_train,X_test
    ...
    def Predict_Novelligands_withSmote_PCA(self,model,X_train,X_test,y_train,ORname):
        pipeline  = sample_pipeline([("HandleMissingValues",HandleMissingValues()),("sampling",SMOTE(random_state=50)),("Normalize",Normalize()),("VarianceFilter",VarianceFilter()),("CorrelationFilter",Correlation_Filter()),('feature_sele',FeatureSelector()),("PCA",pca()),('Model',model)])
        pipeline.fit(X_train,y_train) 
        ligand=X_test['Ligand']  
        X_test=X_test.drop(['Ligand'], axis = 1)
        ...
        Novel_ligands_predictions.to_csv("Predictions_Unknown"+ORname+".csv")
        print("Saved file........."+"Predictions_Unknown"+ORname)
        return pred_labels,prob

    def Predict_Novelligands_withSmote(self,model,X_train,X_test,y_train,ORname):
        pipeline  = sample_pipeline([("HandleMissingValues",HandleMissingValues()),("sampling",SMOTE(random_state=50)),("Normalize",Normalize()),("VarianceFilter",VarianceFilter()),("CorrelationFilter",Correlation_Filter()),('feature_sele',FeatureSelector()),('Model',model)])
        pipeline.fit(X_train,y_train) 
        ligand=X_test['Ligand']  
        X_test=X_test.drop(['Ligand'], axis = 1)
        pred_labels=pipeline.predict(X_test)
...
class HandleDataImbalance:
    ...
    def EvaluatePerformanceSmote(self,trainset,trainlabel,testset,testlabel):
        ...
        for grid_search_model in grid_search_models:
            k=k+1
            pipeline  = sample_pipeline([("HandleMissingValues",HandleMissingValues()),("sampling",SMOTE(random_state=50)),("Normalize",Normalize()),("VarianceFilter",VarianceFilter()),("CorrelationFilter",Correlation_Filter()),('feature_sele',FeatureSelector()),('clf_cv',grid_search_model)])
            pipeline.fit(trainset,trainlabel)
            chosen_features=pipeline.named_steps['feature_sele'].get_feature_names()
            chosen_model=pipeline.named_steps['clf_cv'].best_estimator_
            p2= sample_pipeline([("HandleMissingValues",HandleMissingValues()),("sampling",SMOTE(random_state=50)),("Normalize",Normalize()),("VarianceFilter",VarianceFilter()),("CorrelationFilter",Correlation_Filter()),('feature_sele',FeatureSelector()),("model",pipeline.named_steps['clf_cv'].best_estimator_)])
            best_models.append(("model"+str(k),chosen_model))
            Saving_Model.append(pipeline.named_steps['clf_cv'].best_estimator_) # Appending best estimator
            y_pred_train=pipeline.predict(trainset)
            print("ACCURACY ON TRAIN SET :",accuracy_score(trainlabel,y_pred_train))
        ...
        print("MCC",matthews_corrcoef(testlabel, y_pred_ensemble))
        print("RESULT OF 5 fold Cross Validation")
        p2=sample_pipeline([("HandleMissingValues",HandleMissingValues()),("sampling",SMOTE(random_state=50)),("Normalize",Normalize()),("VarianceFilter",VarianceFilter()),("CorrelationFilter",Correlation_Filter()),('feature_sele',FeatureSelector()),("ENSEMBLE",ensemble)])
        #scoring = {'acc': 'accuracy','prec_macro': 'precision_macro','rec_micro': 'recall_macro'}
        #scores = cross_validate(p2,trainset,trainlabel,scoring=scoring,cv=5, return_train_score=True)
        #print(scores)
        ...
        plt.bar(Models_used,F1_scores_)
        plt.show()
        Figureset.append(fig)
        figcount+=1
        IndexOfBestModel=np.argmax(F1_scores_)
    ...
    def EvaluatePerformanceSmote_PCA(self,trainset,trainlabel,testset,testlabel):
        global figcount
        global Figureset
        svm_gridsearch=Cross_validate_GridSearch().SVM_GridSearch()
        rf_gridsearch=Cross_validate_GridSearch().RandomForest_GridSearch()
        ...
        ensemble = VotingClassifier(best_models,n_jobs=-1,voting='soft')
        Saving_Model.append(ensemble) # Appending Voting model
        pipeline  = sample_pipeline([("HandleMissingValues",HandleMissingValues()),("sampling",SMOTE(random_state=50)),("Normalize",Normalize()),("VarianceFilter",VarianceFilter()),("CorrelationFilter",Correlation_Filter()),('feature_sele',FeatureSelector()),("PCA",pca()),('Ensemble',ensemble)])
        pipeline.fit(trainset,trainlabel)
        y_pred_train=pipeline.predict(trainset)
        y_pred_ensemble=pipeline.predict(testset)
        y_pred_prob_ensemble=pipeline.predict_proba(testset)
        ...
        figcount+=1
        IndexOfBestModel=np.argmax(F1_scores_)
        Plotfigures().PlotAllfigures(Figureset,"Evaluate Different Models with Smote+PCA")
        plt.show()
        return chosen_features,Saving_Model[IndexOfBestModel]
